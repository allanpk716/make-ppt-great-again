...
const CodeHeader = ({ language, code }: { language?: string; code?: string }) => {
  ...
  return (
    <div className="flex items-center justify-between rounded-t-lg bg-muted px-4 py-2">
      ...
      <button onClick={onCopy} className="rounded p-1 hover:bg-muted-foreground/20">
...
const defaultComponents = memoizeMarkdownComponents({
  ...
  code: function Code({ className, ...props }) {
    ...
    return (
      <code
        className={cn(!isCodeBlock && 'rounded bg-muted px-1 py-0.5 font-mono text-sm', className)}
        {...props}
      />
    );
  },
  pre: ({ className, children, ...props }) => {
    ...
    const language =
      (children as React.ReactElement)?.props?.className?.replace('language-', '') || 'code';

    return (
      <div className="mb-2">
        <CodeHeader code={codeContent} language={language} />
        <pre
          className={cn('overflow-x-auto rounded-lg bg-muted p-4 rounded-t-none', className)}
          {...props}
        >
          {children}
        </pre>
